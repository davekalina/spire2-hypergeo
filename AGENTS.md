# Hypergeometric Draw Odds — repository instructions
...
| --- | --- |
| Mod id | `Hypergeo` |
| Display name | Hypergeometric Draw Odds |
| Installs to | `<game>/mods/Hypergeo/` |
| Manifest | `Hypergeo.json` |
| Version is also printed in | `HypergeoCode/MainFile.cs` |
| Gameplay | Informational only; `affects_gameplay` is `false` and must stay `false` |
| Dependencies | None. Do not add BaseLib unless something actually needs it. |

The mod adds native-style draw-chance analysis to the combat pile screens. The core
calculation is `1 - C(N-K, n) / C(N, n)`, implemented in `HypergeoCode/Hypergeometric.cs`
and driven by `HypergeoCode/DrawOddsCalculator.cs`. Both are covered by
`Hypergeo.Tests` and are the one part of this mod that must never regress silently.

Note that `DrawOdds*` type names describe what the mod computes and are intentional.
`Hypergeo` is the mod's identity; `draw odds` is its subject matter.

### Surfaces to audit

Any directional change must be applied across all of these:

- the Draw Pile screen, the Discard Pile screen, and the combined **ALL** screen;
- individual-card odds, selected-card analysis, empty states, and the discard-pile
  boundary marker;
- the **ANY** and **ALL** aggregate calculations and the two `−`/`+` control rows;
- the native bottom information strip, the Draw Chance hover tip, selection outlines,
  and keyboard/controller dismissal.
